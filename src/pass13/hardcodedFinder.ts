...
export class HardcodedFinder extends Finder {
    public find(_: estree.Program, context: Namespace): void {
        ...
        rgb.functions.push(
            ...
            this.getFunction('addColor', [
                ['color', '{r: number, g: number, b: number}'],
                ['strenght', 'number'],
            ])
        )
        rgb.fields.push(this.getField('r', 'number'), this.getField('g', 'number'), this.getField('b', 'number'), this.getField('ok', 'boolean'))
        ns.classes.push(rgb)
        ...
        enemyEntryButton.fields.splice(
            enemyEntryButton.fields.findIndex((f): boolean => f.name === 'level'),
            1
        ) //Remove wrong level entry

        const socialEntryButton = sc.classes.find((c): boolean => c.name === 'SocialEntryButton')
        if (!socialEntryButton) {
            return
        }
        socialEntryButton.fields.splice(
            socialEntryButton.fields.findIndex((f): boolean => f.name === 'level'),
            1
        ) //Remove wrong level entry
